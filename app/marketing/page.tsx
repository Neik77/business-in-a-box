...
export default function MarketingStudio() {
  ...
  const ROOMS = [
    { id: 'brand',    label: 'Brand Message',      num: 1 },
    { id: 'audience', label: 'Audience Messaging',  num: 2 },
    { id: 'plan',     label: 'Marketing Plan',      num: 3 },
    { id: 'content',  label: 'Content Calendar',    num: 4 },
    { id: 'social',   label: 'Social Planning',     num: 5 },
    { id: 'promos',   label: 'Promotions',          num: 6 },
    { id: 'local',    label: 'Local Marketing',     num: 7 },
    { id: 'email',    label: 'Email Marketing',     num: 8 },
    { id: 'coach',    label: 'Marketing Coach',     num: 9 },
  ]
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const welcomes: Record<string, string> = {
        ...
        audience: `Let's get crystal clear on who you are talking to. ${biz} serves ${icp}. What is the one thing your audience needs to hear from you right now?`,
        plan: `A marketing plan is a system for getting your message in front of the right people consistently. What platforms are you currently using to market?`,
        content: `Content is how you build trust before the sale. How many days a week can you realistically create content?`,
        social: `Social media is a tool not a strategy. Where does your ideal customer spend the most time online?`,
        promos: `Promotions create urgency and spike revenue. What upcoming dates, seasons, or milestones can you build promotions around?`,
        local: `Even if ${biz} is primarily online, local visibility can drive serious traffic. Are you visible in your local community and online search?`,
        email: `Email is the highest-converting marketing channel. You own your list - no algorithm can take it from you. Does ${biz} have an email list?`,
        coach: `I am your Marketing Coach for ${biz}. I handle brand messaging, content strategy, social media, promotions, and audience growth. What is your most urgent marketing challenge right now?`,
      }
  ...
  const sendChat = async (roomId: string, messageOverride?: string) => {
    ...
    try {
      ...
      const system = `You are the Marketing Coach inside Business in a Box, built by Coach Neik. You are coaching ${owner}, marketing lead of ${biz}. Their ideal customer: ${icp}. Handle ALL marketing questions - brand messaging, content strategy, social media, email marketing, promotions, audience growth, local marketing. Be direct, specific, platform-aware. No fluff.`
  ...
  const ChatPanel = ({ roomId }: { roomId: string }) => {
    ...
    return (
      <div style={{ marginTop: 24 }}>
        <div style={{ ...s.sectionTitle, marginBottom: 4 }}>Marketing Coach - Ask Anything</div>
          ...
          <div style={{ padding: 20, minHeight: 180, maxHeight: 320, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
            {msgs.map((m, i) => (
              <div key={i} style={{ display: 'flex', gap: 10, alignSelf: m.role === 'user' ? 'flex-end' : 'flex-start', flexDirection: m.role === 'user' ? 'row-reverse' : 'row', maxWidth: '88%' }}>
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: m.role === 'assistant' ? goldDim : '#222', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: white, flexShrink: 0 }}>
                  {m.role === 'assistant' ? 'MK' : (profile?.owner_name?.charAt(0) || 'U')}
                </div>
            ...
            {chatLoading && (
              <div style={{ display: 'flex', gap: 10 }}>
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: goldDim, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: white }}>MK</div>
      ...
      case 'brand': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Your brand message is what you stand for, how you sound, and why people choose you over everyone else.</p>
          </div>
          <div style={s.section}>
          ...
          <div style={s.section}>
            ...
            <label style={s.label}>Current tagline (or what you have been using)</label>
            <input type="text" style={{ ...s.input, resize: undefined }} placeholder="Leave blank if you do not have one yet" />
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('brand'); setActiveRoom('audience') }}>Save and Continue to Audience Messaging</button>
      ...
      case 'audience': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>The right message to the right audience changes everything. Let's nail both.</p>
          </div>
          <div style={s.section}>
            ...
            {['Primary Audience', 'Secondary Audience'].map((label, i) => (
              <div key={i} style={{ background: '#0f0f0f', border: `1px solid ${border}`, borderRadius: 12, padding: 18, marginBottom: 14 }}>
                ...
                <label style={s.label}>Their number one pain point right now</label>
          ...
          <div style={s.section}>
            ...
            <textarea style={s.input} placeholder="e.g. If you have been trying to use AI but it feels overwhelming, this is for you." />
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('audience'); setActiveRoom('plan') }}>Save and Continue to Marketing Plan</button>
      ...
      case 'plan': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>A marketing plan is a system for getting your message in front of the right people consistently.</p>
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>Marketing Overview</div>
          ...
          <div style={s.section}>
            ...
            <p style={s.sectionDesc}>Pick your top 2-3 and commit. You cannot be everywhere at once.</p>
            <label style={s.label}>Primary Channel (where you will focus 80% of your energy)</label>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('plan'); setActiveRoom('content') }}>Save and Continue to Content Calendar</button>
      ...
      case 'content': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Content without a plan is just noise. Let's build a system that creates consistency without burning you out.</p>
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>Content Pillars</div>
            <p style={s.sectionDesc}>The 3-5 topics you talk about consistently. Everything you post falls into one of these.</p>
            {[1, 2, 3, 4, 5].map(i => (
              <div key={i}>
          ...
          <div style={s.section}>
            <div style={s.sectionTitle}>Weekly Content Schedule</div>
          ...
          <div style={s.section}>
            ...
            <label style={s.label}>When will you batch create content?</label>
            <input type="text" style={{ ...s.input, resize: undefined }} placeholder="e.g. Every Sunday 2-4pm, First Saturday of the month..." />
            <label style={s.label}>How many pieces will you create in one batch session?</label>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('content'); setActiveRoom('social') }}>Save and Continue to Social Planning</button>
      ...
      case 'social': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Social media is a tool not a strategy. Let's pick the right platforms and build a plan that actually gets results.</p>
          </div>
          <div style={s.section}>
            ...
            {['TikTok', 'Instagram', 'Facebook', 'YouTube', 'LinkedIn', 'Pinterest'].map(platform => (
              <div key={platform} style={{ background: '#0f0f0f', border: `1px solid ${border}`, borderRadius: 12, padding: 18, marginBottom: 14 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                  ...
                  <div>
                    <label style={{ ...s.label, marginTop: 0 }}>Posts Per Week</label>
                    <input type="text" style={{ ...s.input, resize: undefined }} placeholder="0" />
                  </div>
                </div>
              </div>
          ...
          <div style={s.section}>
            ...
            <label style={s.label}>What is your biggest social media challenge?</label>
            <textarea style={s.input} placeholder="Consistency, ideas, engagement, growth, algorithm, time..." />
          </div>
          <ChatPanel roomId="social" />
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('social'); setActiveRoom('promos') }}>Save and Continue to Promotions</button>
      ...
      case 'promos': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Promotions create urgency and spike revenue. Plan them strategically.</p>
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>Promotion Calendar</div>
            {['Q1 (Jan-Mar)', 'Q2 (Apr-Jun)', 'Q3 (Jul-Sep)', 'Q4 (Oct-Dec)'].map(q => (
              <div key={q}>
                <label style={s.label}>{q} - Key Promotions</label>
                <textarea style={{ ...s.input, minHeight: 70 }} placeholder="e.g. New Year launch, spring sale, back to school, Black Friday, holiday special..." />
              </div>
            ))}
          </div>
          <div style={s.section}>
            ...
            <textarea style={s.input} placeholder="e.g. Flash sales, bundle deals, bonuses for fast action, free challenges, limited enrollment..." />
            <label style={s.label}>What is your policy on discounting?</label>
            <textarea style={s.input} placeholder="Some businesses never discount - they use bonuses instead. What is your approach?" />
          </div>
          <div style={s.section}>
            ...
            <input type="text" style={{ ...s.input, resize: undefined }} placeholder="$0" />
            <label style={s.label}>How will you promote it?</label>
            <textarea style={s.input} placeholder="e.g. 3 TikToks, 5 Instagram stories, 2 emails, DMs to warm leads..." />
          </div>
          <ChatPanel roomId="promos" />
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('promos'); setActiveRoom('local') }}>Save and Continue to Local Marketing</button>
      ...
      case 'local': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Even if {profile?.business_name} is primarily online, local visibility can drive serious traffic.</p>
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>Local Presence</div>
          ...
          <div style={s.section}>
            ...
            <input type="text" style={{ ...s.input, resize: undefined }} placeholder="e.g. Set up Google Business Profile, join 2 local networking groups..." />
          </div>
          <ChatPanel roomId="local" />
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('local'); setActiveRoom('email') }}>Save and Continue to Email Marketing</button>
      ...
      case 'email': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Your email list is the one audience you actually own. No algorithm. No platform risk.</p>
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>Email Foundation</div>
            <label style={s.label}>Email platform you are using or plan to use</label>
            <select style={{ ...s.input, resize: undefined }}>
          ...
          <div style={s.section}>
            ...
            <label style={s.label}>What is your lead magnet?</label>
          ...
          <div style={s.section}>
            <div style={s.sectionTitle}>Email Strategy</div>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('email'); setActiveRoom('coach') }}>Save and Continue to Marketing Coach</button>
      ...
      case 'coach': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Ask anything about brand, content, social media, email, promotions, and audience growth.</p>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24 }}>
            {[
              ...
              { num: '02', title: 'Content Must Drive to an Offer', body: 'Every piece of content you create should eventually point to something people can buy. If your content does not connect to your business, it is just content - not marketing.' },
    ...
    <div style={{ minHeight: '100vh', background: bg, color: white, fontFamily: 'DM Sans,sans-serif' }}>
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 28px', borderBottom: `1px solid ${border}`, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(12px)', position: 'sticky', top: 0, zIndex: 200, gap: 12, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 38, height: 38, background: '#111', border: `1.5px solid ${gold}`, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: gold }}>MKT</div>
          <div>
