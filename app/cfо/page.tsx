...
export default function CFOMoneyRoom() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const welcomes: Record<string, string> = {
        ...
        expenses: `Let's track every dollar going OUT of ${biz}. Knowing your expenses is just as important as knowing your income. What are your top monthly expenses right now?`,
        pricing: `Pricing is where most small business owners leave money on the table. Let's make sure ${biz} is priced for profit, not just to stay busy. What are you currently charging and do you feel like it's enough?`,
        budget: `A budget is just a plan for your money before it arrives. Let's build one for ${biz}. What does a typical month look like in terms of what you need to spend to operate?`,
        profit: `Profit is what's left after everything is paid. Let's plan for it intentionally. Right now, what percentage of your revenue do you think you're actually keeping as profit?`,
        cashflow: `Cash flow is the lifeblood of any business. You can be profitable on paper and still broke in real life. Let's map out when money comes in and when it goes out for ${biz}.`,
        breakeven: `Your break-even point is the minimum you need to cover all costs. Once you know it, everything above it is profit. Let's calculate it for ${biz}.`,
        goals: `Let's set your financial goals — not wishes, real targets with numbers and dates. What does financial success look like for ${biz} in the next 12 months?`,
        coach: `I'm your CFO Coach for ${biz}. I handle money, pricing, profit, and financial strategy. CEO Coach handles business direction. What's your most urgent money question right now?`,
      }
  ...
  const saveRoom = async (roomId: string) => {
    ...
    await supabase.from('profiles').update({
      rooms_unlocked: JSON.stringify([...updated].map(r => r)),
    }).eq('id', user?.id)
    showToast()
  }
  ...
  const sendChat = async (roomId: string, messageOverride?: string) => {
    ...
    try {
      const biz = profile?.business_name || 'your business'
      const owner = profile?.owner_name?.split(' ')[0] || 'CEO'
      const system = `You are the CFO Coach inside Business in a Box™, built by Coach Neik™. You are coaching ${owner}, the CFO of ${biz}. You handle ALL financial questions — income, expenses, pricing, profit, cash flow, budgeting, break-even, and financial goals. Be direct, specific, and use real numbers. No fluff. Short punchy responses. If asked about business strategy route to CEO Coach. If asked about legal/setup route to Compliance Coach (V6). If asked about funding/credit route to Funding Coach (V7).`
      ...
      setChatMessages(prev => ({ ...prev, [roomId]: [...(prev[roomId] || []), { role: 'assistant', content: data.reply || "Let's work through this together." }] }))
    } catch {
  ...
  const ChatPanel = ({ roomId }: { roomId: string }) => {
    ...
    const prompts: Record<string, string[]> = {
      ...
      expenses: ['What expenses can I cut?', 'What are normal business expenses?', 'How do I separate personal and business expenses?'],
      pricing: ['Am I charging enough?', 'How do I raise my prices?', 'What pricing model works best for my business?'],
      ...
      cashflow: ['How do I fix a cash flow problem?', 'When should I expect slow months?', 'How do I build a cash reserve?'],
    ...
    return (
      <div style={{ marginTop: 24 }}>
        <div style={{ ...s.sectionTitle, marginBottom: 4 }}>💰 Your CFO Coach</div>
  ...
  const renderRoom = () => {
    switch (activeRoom) {
      ...
      case 'income': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>You can't grow what you don't measure. Let's get every dollar that comes into {profile?.business_name} documented and organized.</p>
          </div>
          <div style={s.section}>
          ...
          <div style={s.section}>
            ...
            <p style={s.sectionDesc}>What does a typical month look like?</p>
            ...
            <label style={s.label}>When are your slowest months? (seasonality)</label>
            ...
            <textarea style={s.input} placeholder="Be specific — more clients, higher prices, new offers, more content..." />
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>🔄 Recurring vs One-Time Income</div>
            <p style={s.sectionDesc}>Recurring income is predictable. One-time income is unpredictable. You need both.</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div>
                <label style={s.label}>Monthly Recurring Revenue (MRR)</label>
                <input type="text" style={{ ...s.input, resize: undefined }} placeholder="$0/month guaranteed" />
              </div>
              <div>
                <label style={s.label}>One-Time Revenue (avg/month)</label>
                <input type="text" style={{ ...s.input, resize: undefined }} placeholder="$0/month estimated" />
              </div>
            </div>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('income'); setActiveRoom('expenses') }}>Save & Continue to Expense Tracking →</button>
      ...
      case 'expenses': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Every dollar out needs to be accounted for. Let's find where your money is going and whether it's working for you.</p>
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>📋 Fixed Monthly Expenses</div>
            <p style={s.sectionDesc}>These are the same every month — they don't change based on revenue.</p>
            {['Software & Subscriptions', 'Phone & Internet', 'Storage / Workspace', 'Insurance', 'Loan / Debt Payments', 'Other Fixed'].map((label, i) => (
              <div key={i}>
                <label style={s.label}>{label}</label>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12 }}>
          ...
          <div style={s.section}>
            ...
            <p style={s.sectionDesc}>These change month to month based on activity and revenue.</p>
            {['Marketing & Advertising', 'Supplies & Materials', 'Contractor / Freelancer Costs', 'Shipping & Fulfillment', 'Professional Development', 'Other Variable'].map((label, i) => (
              <div key={i}>
                <label style={s.label}>{label}</label>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12 }}>
                  <input type="text" style={{ ...s.input, resize: undefined }} placeholder="Description..." />
          ...
          <div style={s.section}>
            <div style={s.sectionTitle}>✂️ Expense Audit</div>
            <p style={s.sectionDesc}>Not every expense is worth keeping. Let's audit.</p>
            <label style={s.label}>Subscriptions or expenses you're not sure you need</label>
            <textarea style={s.input} placeholder="List anything you're paying for but not fully using..." />
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('expenses'); setActiveRoom('pricing') }}>Save & Continue to Pricing →</button>
      ...
      case 'pricing': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            <div style={{ fontFamily: 'monospace', fontSize: 10, letterSpacing: '2.5px', textTransform: 'uppercase', color: gold, marginBottom: 8 }}>🏷️ Room 3 of 9 — Pricing</div>
            <h1 style={{ fontFamily: 'Georgia,serif', fontSize: 'clamp(24px,4vw,32px)', fontWeight: 900, marginBottom: 8 }}>Price for Profit</h1>
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Most small business owners underprice and overdeliver. Let's fix that. Your price needs to cover your costs, pay you properly, and reflect your value.</p>
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>💡 Pricing Philosophy</div>
            <label style={s.label}>How did you come up with your current prices?</label>
            <textarea style={s.input} placeholder="e.g. Guessed, looked at competitors, calculated costs, coach told me..." />
            <label style={s.label}>Do you feel like you're charging enough? Why or why not?</label>
          ...
          <div style={s.section}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
              ...
              <div><label style={s.label}>Hours Available/Week</label><input type="text" style={{ ...s.input, resize: undefined }} placeholder="20 hrs" /></div>
            </div>
            <label style={s.label}>Minimum hourly rate needed (income goal + expenses ÷ hours)</label>
            <input type="text" style={{ ...s.input, resize: undefined }} placeholder="Calculate: ($5,000 + $1,500) ÷ 80 hrs/month = $81.25/hr minimum" />
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>📦 Offer Pricing Review</div>
            <p style={s.sectionDesc}>List your current offers and what they cost vs what they should cost.</p>
            {[1, 2, 3].map(i => (
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('pricing'); setActiveRoom('budget') }}>Save & Continue to Budget →</button>
      ...
      case 'budget': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>A budget tells your money where to go before it disappears. Let's build a monthly budget that works for {profile?.business_name}.</p>
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>📥 Monthly Budget Plan</div>
            <p style={s.sectionDesc}>Allocate your projected monthly revenue across these categories.</p>
            {[
              ...
              { label: 'Education & Growth', placeholder: 'Courses, coaching, books...' },
          ...
          <div style={s.section}>
            <div style={s.sectionTitle}>⚠️ Budget Blockers</div>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('budget'); setActiveRoom('profit') }}>Save & Continue to Profit Planning →</button>
      ...
      case 'profit': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Revenue is vanity. Profit is sanity. Let's make sure {profile?.business_name} is built to actually keep money, not just make it.</p>
          </div>
          <div style={s.section}>
            ...
            <label style={s.label}>Current profit margin % (profit ÷ revenue × 100)</label>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('profit'); setActiveRoom('cashflow') }}>Save & Continue to Cash Flow →</button>
      ...
      case 'cashflow': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>You can be profitable on paper and still broke. Cash flow is about timing — when money comes in vs when bills go out.</p>
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>📅 Cash Flow Calendar</div>
            <p style={s.sectionDesc}>Map out when money typically comes in and when bills go out.</p>
            <label style={s.label}>When does most of your income arrive? (beginning, middle, end of month)</label>
          ...
          <div style={s.section}>
            ...
            <label style={s.label}>What's your plan to build a 3-month cash reserve?</label>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('cashflow'); setActiveRoom('breakeven') }}>Save & Continue to Break-Even →</button>
      ...
      case 'breakeven': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            <div style={{ fontFamily: 'monospace', fontSize: 10, letterSpacing: '2.5px', textTransform: 'uppercase', color: gold, marginBottom: 8 }}>⚖️ Room 7 of 9 — Break-Even</div>
            <h1 style={{ fontFamily: 'Georgia,serif', fontSize: 'clamp(24px,4vw,32px)', fontWeight: 900, marginBottom: 8 }}>Know Your Break-Even</h1>
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Your break-even is the minimum you need to cover all costs. Once you know it, everything above it is profit. Let's calculate it.</p>
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>🧮 Break-Even Calculator</div>
            <p style={s.sectionDesc}>Fill these in and your CFO Coach will help you calculate your break-even point.</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div><label style={s.label}>Average Revenue Per Client/Sale</label><input type="text" style={{ ...s.input, resize: undefined }} placeholder="$0" /></div>
              <div><label style={s.label}>Cost to Deliver Each Sale</label><input type="text" style={{ ...s.input, resize: undefined }} placeholder="$0 (time + materials)" /></div>
              <div><label style={s.label}>Gross Profit Per Sale</label><input type="text" style={{ ...s.input, resize: undefined }} placeholder="Revenue - Cost to Deliver" /></div>
            </div>
            <label style={s.label}>Estimated Break-Even (Fixed Costs ÷ Gross Profit Per Sale)</label>
            <input type="text" style={{ ...s.input, resize: undefined }} placeholder="e.g. $2,000 ÷ $200 = 10 sales needed to break even" />
          </div>
          <div style={s.section}>
            ...
            <input type="text" style={{ ...s.input, resize: undefined }} placeholder="e.g. 8 template sales at $97 = $776 (not enough), need 21 sales to hit $2,000" />
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('breakeven'); setActiveRoom('goals') }}>Save & Continue to Financial Goals →</button>
      ...
      case 'goals': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Real financial goals have numbers and dates. Let's set them for {profile?.business_name} across short, medium, and long term.</p>
          </div>
          <div style={s.section}>
            <div style={s.sectionTitle}>🗓️ Short-Term Goals (Next 90 Days)</div>
            ...
            <input type="text" style={{ ...s.input, resize: undefined }} placeholder="e.g. Weekly money check every Sunday, separate business account, track every expense" />
          </div>
          <div style={s.section}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button style={s.btnPrimary} onClick={() => { saveRoom('goals'); setActiveRoom('coach') }}>Save & Continue to CFO Coach →</button>
      ...
      case 'coach': return (
        <div>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 14, color: muted, lineHeight: 1.65, maxWidth: 560 }}>Ask anything about money, pricing, profit, cash flow, and financial strategy for {profile?.business_name}. Direct answers, no fluff.</p>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24 }}>
            {[
              { num: '01', title: 'Know Your Numbers Weekly', body: 'Set up a 10-minute weekly money check every Sunday. Revenue in, expenses out, profit kept. That\'s it. Consistency beats perfection.' },
              { num: '02', title: 'Separate Business and Personal', body: 'If you don\'t have a separate business bank account, get one this week. Mixing money is the #1 reason small business owners can\'t see their real profit.' },
              { num: '03', title: 'Price for Profit, Not Survival', body: 'Your price should cover your costs, pay you a real wage, AND leave profit. If it doesn\'t, raise your price or cut your costs — there\'s no other option.' },
            ].map(tip => (
