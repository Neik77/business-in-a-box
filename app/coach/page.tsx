...
export default function CoachDashboard() {
  ...
  if (loading) return (
    <div style={{minHeight:'100vh',background:'#0A0A0C',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{color:'#D4AF37',fontFamily:'Georgia,serif',fontSize:18}} role="status">Loading coach dashboard…</div>
    </div>
  )
    ...
    <div className="page-outer">
      <div style={{maxWidth:1100,margin:'0 auto'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:32,flexWrap:'wrap',gap:12}}>
          <div>
            <h1 style={{fontFamily:'Georgia,serif',fontSize:32,margin:'0'}}>Client <span style={{color:'#D4AF37'}}>Headquarters</span></h1>
            <p style={{color:'#9B968A',fontSize:13,margin:'6px 0 0'}}>Coach dashboard</p>
          </div>
          <button onClick={() => router.push('/dashboard')} className="btn btn-ghost">My Dashboard</button>
        </div>

        <div className="coach-grid" style={{gridTemplateColumns:selected?'320px 1fr':'1fr'}}>
          <div>
            <h2 style={{fontSize:14,letterSpacing:'.1em',textTransform:'uppercase',color:'#9B968A',marginBottom:16}}>
              All Clients ({clients.length})
            </h2>
            {clients.length === 0 && (
              <p style={{color:'#9B968A',fontSize:13,fontStyle:'italic'}}>No clients yet.</p>
            )}
            {clients.map(c => (
              <button
                key={c.id}
                ...
                aria-label={`View ${c.business_name || 'Unnamed Business'} — ${c.owner_name || c.email}`}
              >
                ...
                <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                  <span style={{fontSize:11,background:c.intake_done?'rgba(212,175,55,0.2)':'rgba(255,255,255,0.05)',color:c.intake_done?'#D4AF37':'#9B968A',padding:'3px 8px',borderRadius:999}}>
                    {c.intake_done ? 'Intake Done' : 'No Intake'}
                  </span>
                  {c.business_stage && (
                    <span style={{fontSize:11,background:'rgba(255,255,255,0.05)',color:'#9B968A',padding:'3px 8px',borderRadius:999}}>
                      {c.business_stage}
                    </span>
                  )}
                </div>
            ...
            <div>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
                <h2 style={{fontFamily:'Georgia,serif',fontSize:22,margin:0}}>
                  {selected.business_name} <span style={{color:'#D4AF37'}}>File</span>
                </h2>
                <button onClick={() => setSelected(null)} className="btn btn-ghost" style={{padding:'6px 12px',fontSize:12}}>Close</button>
              </div>
                  ...
                  <div className="detail-grid-2">
                    {[
                      ...
                      <div key={label as string}>
                        <span style={labelStyle}>{label}</span>
                        <p style={{margin:0,fontSize:13,color:val?'#DAD6CB':'#9B968A'}}>{val || 'Not completed yet'}</p>
                      </div>
                    ))}
                  </div>
              ...
              <div style={cardStyle}>
                ...
                <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
                  ...
                  {badges.length === 0 && (
                    <span style={{color:'#9B968A',fontSize:13}}>No badges earned yet</span>
                  )}
                </div>
              </div>
            </div>
